...
export function NotificationsSection() {
  ...
  if (!user && hydrated) {
    return <p style={{ fontSize: "13px", color: "var(--m-ink-soft)" }}>Sign in to manage notifications.</p>;
  }

  if (loading) {
    return <div className="animate-pulse h-40 rounded-2xl" style={{ background: "var(--m-card)" }} />;
  }

  return (
    <div className="space-y-4">
      <div className="p-4" style={{ background: "var(--m-card)", border: "1px solid var(--m-ink-faint)", borderRadius: "16px" }}>
        <h2 style={{ fontSize: "15px", fontWeight: 700, color: "var(--m-ink)" }}>Daily nudges</h2>
        <p style={{ fontSize: "12px", color: "var(--m-ink-soft)", marginTop: "4px", lineHeight: 1.5 }}>
...
function SmallButton({ onClick, loading, icon: Icon, children }: { onClick: () => void; loading: boolean; icon: React.ElementType; children: React.ReactNode }) {
  return (
    <button onClick={onClick} disabled={loading} className="inline-flex items-center gap-1.5 px-3 py-1.5 mt-1 transition-colors"
      style={{ fontSize: "12px", fontWeight: 600, background: "var(--m-cream-2)", color: "var(--m-ink)", border: "1px solid var(--m-ink-faint)", borderRadius: "980px" }}>
...
function SwiggyCard({ status, busy, onConnect, onDisconnect }: { status: SwiggyStatus | null; busy: boolean; onConnect: () => void; onDisconnect: () => void }) {
  if (!status) return <div className="animate-pulse h-24 rounded-2xl" style={{ background: "var(--m-card)" }} />;
  ...
  return (
    <div className="p-4" style={{ background: "var(--m-card)", border: "1px solid var(--m-ink-faint)", borderRadius: "16px" }}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <h3 style={{ fontSize: "14px", fontWeight: 700, color: "var(--m-ink)" }}>Swiggy agent</h3>
            <StatusPill tone={connected ? "active" : "off"}>
              {connected ? "Connected" : "Not connected"}
            </StatusPill>
          </div>
          <p style={{ fontSize: "11px", color: "var(--m-ink-soft)", marginTop: "2px" }}>
...
function WhatsAppDetail({ sub, phoneInput, setPhoneInput, enrollLoading, joinInstructions, onEnroll, onResendJoin, onTest, testing, onCopyJoin }: {
  ...
  if (sub.whatsappStatus === "pending") {
    return (
      <div className="flex flex-col gap-1.5 mt-1">
        <p style={{ fontSize: "11px", color: "var(--m-ink-soft)" }}>Send this JOIN code on WhatsApp:</p>
        {joinInstructions ? (
          <div className="flex items-center gap-2">
            <code className="px-2 py-1 font-mono" style={{ fontSize: "12px", background: "var(--m-cream-2)", borderRadius: "6px", color: "var(--m-ink)" }}>{joinInstructions.text}</code>
            <button onClick={onCopyJoin} className="px-2 py-1" style={{ fontSize: "11px", background: "var(--m-cream-2)", border: "1px solid var(--m-ink-faint)", borderRadius: "999px", color: "var(--m-ink)" }}>
  ...
  if (sub.whatsappStatus === "active") {
    return (
      <div className="mt-1">
        <p style={{ fontSize: "11px", color: "var(--m-ink-soft)" }}>Connected on <strong>{sub.phoneE164}</strong></p>
