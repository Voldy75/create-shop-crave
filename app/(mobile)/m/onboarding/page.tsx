...
export default function Onboarding() {
  ...
  return (
    <div className="col" style={{ minHeight: "100dvh", background: "var(--cc-bg)" }}>
      <div className="row" style={{ padding: "calc(env(safe-area-inset-top,12px) + 8px) 22px 14px", justifyContent: "space-between", alignItems: "center" }}>
        <button onClick={back} style={{ background: "none", border: "none", color: "var(--cc-ink-2)" }} aria-label="Back">
          <ChevronLeft width={22} height={22} />
        </button>
        <div className="row" style={{ gap: 4 }}>
          {STEPS.slice(1).map((_, i) => (
            <div key={i} style={{ width: 22, height: 3, borderRadius: 2, background: i <= idx - 1 ? "var(--cc-acc)" : "var(--cc-line-2)" }} />
          ))}
        </div>
        {step !== "signup" ? (
          <button onClick={next} style={{ background: "none", border: "none", color: "var(--cc-ink-3)", fontSize: 13 }}>Skip</button>
        ) : (
          <span style={{ width: 36 }} />
        )}
      </div>
      ...
      {step === "location" && (
        <>
          <div className="col" style={{ flex: 1, justifyContent: "center", alignItems: "center", textAlign: "center", gap: 18, padding: "0 22px" }}>
            <div style={{ width: 100, height: 100, borderRadius: "50%", background: "var(--cc-acc-dim)", display: "grid", placeItems: "center", position: "relative" }}>
              <MapPin width={44} height={44} style={{ color: "var(--cc-acc)" }} />
              <div style={{ position: "absolute", inset: -10, borderRadius: "50%", border: "1.5px solid var(--cc-acc-soft)" }} />
              <div style={{ position: "absolute", inset: -20, borderRadius: "50%", border: "1px solid var(--cc-acc-soft)" }} />
            </div>
            <div className="col" style={{ gap: 8, alignItems: "center" }}>
              <h1 className="t-h1">Where are you?</h1>
              <p className="t-body" style={{ color: "var(--cc-ink-2)", maxWidth: 260 }}>
      ...
      {step === "diet" && (
        <>
          <div style={{ padding: "0 22px 8px" }}>
            ...
            <p className="t-body" style={{ color: "var(--cc-ink-2)", marginTop: 6 }}>Pick any that apply.</p>
          </div>
      ...
      {step === "goals" && (
        <>
          <div style={{ padding: "0 22px 8px" }}>
            ...
            <p className="t-body" style={{ color: "var(--cc-ink-2)", marginTop: 6 }}>We&apos;ll set a starting calorie target — tweak it anytime.</p>
          </div>
            ...
            {GOAL_OPTIONS.map((g) => {
              ...
              return (
                <button
                  ...
                  className="card row"
                  style={{ padding: 16, gap: 12, justifyContent: "space-between", borderColor: on ? "var(--cc-acc)" : "var(--cc-line)", background: on ? "var(--cc-acc-dim)" : "var(--cc-surf-1)" }}
                >
                  <div className="col" style={{ gap: 2, alignItems: "flex-start" }}>
                    <span className="t-h3">{g.label}</span>
                    <span className="t-small">{g.sub}</span>
                  </div>
                  <div style={{ width: 22, height: 22, borderRadius: "50%", border: on ? "none" : "1.5px solid var(--cc-line-2)", background: on ? "var(--cc-acc)" : "transparent", display: "grid", placeItems: "center" }}>
      ...
      {step === "signup" && (
        <div className="col" style={{ flex: 1, padding: "8px 22px 24px" }}>
          ...
          <p className="t-body" style={{ color: "var(--cc-ink-2)", marginTop: 10, marginBottom: 28 }}>So your taste follows you across devices.</p>
          <div className="col" style={{ gap: 10 }}>
          ...
          <p className="t-small" style={{ marginTop: "auto", textAlign: "center", color: "var(--cc-ink-3)" }}>
