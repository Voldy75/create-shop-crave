    ...
    <div className="col" style={{ minHeight: "100dvh", background: "var(--cc-bg)" }}>
      ...
      <div className="scroll" style={{ flex: 1, padding: "14px 14px 24px" }}>
        ...
        <div className="col" style={{ gap: 12 }}>
          {platforms.map((p) => (
            <div key={p.id} className="card" style={{ padding: 14, position: "relative", borderColor: p.best ? "var(--cc-acc)" : "var(--cc-line)", borderWidth: p.best ? 1.5 : 1 }}>
              {p.badge && (
                <span style={{ position: "absolute", top: -8, left: 14, background: "var(--cc-acc)", color: "#fff", fontSize: 9, fontWeight: 700, letterSpacing: "0.07em", padding: "3px 8px", borderRadius: 4 }}>{p.badge}</span>
              )}
              <div className="row" style={{ justifyContent: "space-between", alignItems: "baseline" }}>
                <span style={{ fontSize: 15, fontWeight: 600 }}>{p.name}</span>
                <span className="row" style={{ gap: 4, color: "var(--cc-ink-2)" }}><Zap width={12} height={12} style={{ color: "var(--cc-acc)" }} /><span style={{ fontSize: 13, fontWeight: 600 }}>{p.eta}</span></span>
              </div>
        ...
        <div className="card" style={{ marginTop: 16, padding: 14, display: "flex", gap: 10, alignItems: "flex-start", background: "var(--cc-acc-soft)", borderColor: "var(--cc-acc-dim)" }}>
          <Check width={18} height={18} style={{ color: "var(--cc-acc)", flexShrink: 0, marginTop: 1 }} />
          <span className="t-small" style={{ color: "var(--cc-ink-1)" }}>
